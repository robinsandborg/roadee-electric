...
export default function PostCard(props: PostCardProps) {
  ...
  return (
    <article className="rounded-2xl border border-chip-line bg-island-bg p-4 shadow-island-shadow sm:p-5">
      <div className="flex flex-wrap items-center gap-2 text-xs">
        ...
        {props.item.tags.slice(0, 4).map((tag) => (
          <span
            key={tag.id}
            className="rounded-full border border-chip-line bg-chip-bg px-2.5 py-1 text-[11px]"
          >
            #{tag.name}
          </span>
        ))}
      ...
      <h3 className="mt-3 text-xl leading-tight font-bold text-sea-ink">
        <Link
      ...
      <p className="mt-2 line-clamp-3 text-sm leading-6 text-sea-ink-soft">
        {preview || "No body content provided yet."}
      </p>

      <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-sea-ink-soft">
